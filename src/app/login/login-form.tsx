...
export function LoginForm({ initialMessage, initialError }: LoginFormProps) {
  ...
  const [submitType, setSubmitType] = useState<'login' | 'signup'>('login')
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        if (type === 'login') {
          // Success login: redirect via full reload to verify new proxy session
  ...
  return (
    <div className="w-full max-w-md p-8 bg-zinc-900/60 backdrop-blur-xl border border-zinc-800 rounded-2xl shadow-2xl relative overflow-hidden">
      ...
      <div className="relative my-6 flex items-center justify-center">
        ...
        <span className="relative px-3 bg-zinc-950 text-[10px] uppercase tracking-widest text-zinc-500">
          Or continue with
        </span>
      </div>
      ...
      <button
        ...
        disabled={loading !== null}
        className="w-full flex items-center justify-center gap-2.5 px-4 py-2.5 bg-zinc-950 hover:bg-zinc-900 border border-zinc-800 rounded-lg text-sm text-zinc-300 font-medium transition-all focus:outline-none focus:ring-2 focus:ring-zinc-800 disabled:opacity-50 cursor-pointer"
      >
        {loading === 'google' ? (
